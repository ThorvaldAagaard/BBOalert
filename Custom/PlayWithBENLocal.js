...
BENsTurnToPlay = function (overlay) {
	...
	try {
		...
		if (deal["played"].length == 0) {
			var url = "http://localhost:8085/lead?user=" + user + "&dealer=" + dealer + "&dealno=" + dealnumber + "&seat=" + seat + "&vul=" + vul + "&ctx=" + ctx + "&hand=" + hand;
			...
			var url = "http://localhost:8085/play?user=" + user + "&dealer=" + dealer + "&dealno=" + dealnumber + "&seat=" + seat + "&vul=" + vul + "&ctx=" + ctx + "&hand=" + hand +
		...
		try {
			fetch(url, {
				...
				.then(function (data) {
					// Proceed with the logic if the response was successful
					if (data.message) {
						console.log(getNow(true) + " onMyTurnToPlay BEN return message:",data.message)
					} else {
						if (data.claim) {
